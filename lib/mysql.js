...
function initialize(params) {
	...
	function extend(attributes) {
		...
		Model.prototype.update = function update(where_clause, set_fields) {
			...
			return _knex(self.tableName).where(CommonHelper.deleteAllPropertiesWhichAreUndefined(self.getDBObject(where_clause))).update(CommonHelper.deleteAllPropertiesWhichAreUndefined(self.preUpdate(self.getDBObject(set_fields))));

		};

		Model.prototype.save = function save() {
			var self = this;
			return _knex.raw(getInsertSQL(self, CommonHelper.deleteAllPropertiesWhichAreUndefined(self.preSave(self.getDBObject(self.input)))));
		};
		...
		Model.prototype.saveInBatch = function saveInBatch() {
			...
			for (var index = 0; index < self.input.length; index++) {
				batch_statement += getInsertSQL(self, CommonHelper.deleteAllPropertiesWhichAreUndefined(self.preSave(self.getDBObject(self.input[index]))));
			}
		...
		Model.prototype.deleteObject = function deleteObject() {
			...
			return _knex(self.tableName).where(
				CommonHelper.deleteAllPropertiesWhichAreUndefined(self.preDelete(self.getDBObject(self.input)))).del();
		};
